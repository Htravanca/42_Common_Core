...
/*   Updated: 2025/02/23 14:47:37 by hepereir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

//orders the fork to prevent lock order violation helgrind
...
static void ft_orderforks(pthread_mutex_t **first, pthread_mutex_t **second, int first_id, int second_id)
{
    pthread_mutex_t *temp;
    int             temp_id;
...
static void	ft_initphilos_aux(pthread_mutex_t *forks)
{
	...
	while (i < get_data()->num_philos)
	{
        ...
        get_data()->philos[i].l_fork_id = i;  // Assign fork ID
        if (i == 0)
        {
            get_data()->philos[i].r_fork = &forks[get_data()->num_philos - 1];
            get_data()->philos[i].r_fork_id = get_data()->num_philos - 1;  // Assign fork ID
        }
        else
        {
            get_data()->philos[i].r_fork = &forks[i - 1];
            get_data()->philos[i].r_fork_id = i - 1;  // Assign fork ID
        }
        ft_orderforks(&get_data()->philos[i].l_fork, &get_data()->philos[i].r_fork,
                      get_data()->philos[i].l_fork_id, get_data()->philos[i].r_fork_id);
		i++;
	}
}
